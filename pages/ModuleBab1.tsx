...
const pdfPath = 'gs://rwikistatweb.appspot.com/ModulesPDFs/Modul BAB I.pdf';

type ModuleBab1Props = {
  showUnauthorizedAccessWarning?: boolean;
};

const ModuleBab1: React.FC<ModuleBab1Props> = ({ showUnauthorizedAccessWarning = false }) => {
  const [open, setOpen] = useState(showUnauthorizedAccessWarning);
  const { user, loading } = useAuth();
  const router = useRouter();
  const [pdfUrl, setPdfUrl] = useState<string | null>(null); 

  const pdfPath = 'ModulesPDFs/Modul BAB I.pdf';
    ...
    <div className="ml-3 flex bg-white flex-col items-center">  
      ...
      <div className="pl-[998px] drop-shadow-xl">
        <button
              ...
              className="flex flex-row  item-center mb-4 bg-[#eb3323cc] hover:bg-white hover:text-black hover:border-2 hover:border-black  text-white font-bold py-2 px-4 rounded w-[400px] h-[40px]">
        ...
        <div className="pl-[30px] text-[18px]">
          ...
          <h2 className="bg-gray-200 border-l-8 border-[#2d7343ac]  font-bold pt-3 pb-3 pl-2 text-[21px]">1.2.	Perintah Dasar dalam Pemrograman R</h2> 
          <div className="pl-3 pb-6 pt-3">
            <p className="text-justify pl-4 indent-first-line">
...
export default withAuth(ModuleBab1);