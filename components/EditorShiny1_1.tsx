...
server <- function(input, output) {
  ...
  output$scatterPlot <- renderPlot({
    ...
    plot(data$x, data$y, col = 'darkblue', pch = 16, cex = 1.5, xlab = 'X', ylab = 'Y')
  })
}
...
shinyApp(ui = ui, server = server)`);
  ...
  const showPlot = () => {
    ...
    setIframeSrc("https://testnaufal.shinyapps.io/interactiveplot1_1/");
  }; 
      ...
      />
        ...
        <div className="flex flex-col border-4 w-[50%] rounded-2xl outputShiny">
          ...
          <div className="output relative flex "style={{height: '100%'}}>
            {iframeSrc !== "" ? 
              <iframe src={iframeSrc}  style={{width: '10%', height: '100%', flex: '1'}}></iframe> 
              : output}
          </div>
        </div>
      </div>
