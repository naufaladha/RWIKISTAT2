...
const EditorShiny = () => {
  ...
  ui <- fluidPage(
    titlePanel("Histogram of Blood Cell Volume"),
    sidebarLayout(
      sidebarPanel(
  ...
  server <- function(input, output) {
    output$hist <- renderPlot({
      selDarah <- rnorm(50, mean = 10, sd = 2)
      bins <- input$bins
      hist(selDarah,
  ...
  const showPlot = () => {
    ...
    setIframeSrc("https://testnaufal.shinyapps.io/shinyinteractive1_3/");
  }; 
      ...
      />
        ...
        <div className="flex flex-col border-4 w-[50%] rounded-2xl outputShiny">
          ...
          <div className="output relative flex "style={{height: '100%'}}>
            {iframeSrc !== "" ? 
              <iframe src={iframeSrc}  style={{width: '50%', height: '100%', flex: '1'}}></iframe> 
              : output}
          </div>
        </div>
      </div>
